...
/**
 * LH DB 어댑터 (Fallback용)
 * feature.subscription.lh-db-enabled=true 일 때만 활성화
 */
@Slf4j
@Component
@ConditionalOnProperty(
    name = "feature.subscription.lh-db-enabled",
    havingValue = "true",
    matchIfMissing = false
)
@RequiredArgsConstructor
public class LhDbAdapter implements SubscriptionInnerWorldProvider {

    private static final String SOURCE_PREFIX = "LH_API";
    private final SubscriptionRepository subscriptionRepository;

    @Override
    public List<SubscriptionInfo> fetch(String areaName, LocalDate targetDate) {
        try {
            log.info("[LH DB] {} 지역 금일 이후 접수 시작 청약 조회", areaName);
            ...
            log.info("[LH DB] {} 지역에서 {}개 데이터 조회 완료", areaName, subscriptions.size());
            return subscriptions;

        } catch (Exception e) {
            log.error("[LH DB] 데이터 조회 실패: {}", e.getMessage(), e);
